...
async function fetchCategories(): Promise<PublicCatalogCategory[]> {
  ...
  const [categories, subcategories] = await Promise.all([
    db
      .select({
        ...
        h1Override: categoriesTable.h1Override,
    ...
    db
      .select({
        ...
        h1Override: subcategoriesTable.h1Override,
        seoTitle: subcategoriesTable.seoTitle,
  ...
  for (const subcategory of subcategories) {
    ...
    arr.push({
      ...
      h1Override: subcategory.h1Override ?? undefined,
      seoTitle: subcategory.seoTitle ?? undefined,
  ...
  return getOrderedCatalogCategories(
    categories.map((category) => ({
      ...
      name: category.name,
      slug: category.slug,
      h1Override: category.h1Override ?? undefined,
